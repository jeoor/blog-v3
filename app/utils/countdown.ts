...
export function getCountdownProgress(unit: CountdownUnitKey, now: Date): { remaining: number, percentage: number } {
	...
	return calculators[unit](now)
}

function getSpringFestivalDate(year: number): Date | null {
	if (year < MIN_YEAR || year > MAX_YEAR) {
		return null
	}

	let offsetDays = 0
	for (let current = MIN_YEAR; current < year; current += 1) {
...
function getLunarYearDaysCount(year: number): number {
	const yearInfo = LUNAR_INFO[year - MIN_YEAR] ?? 0
	let sum = 348
...
function getDayProgress(now: Date): { remaining: number, percentage: number } {
	const hours = now.getHours()
	return { remaining: 24 - hours, percentage: (hours / 24) * 100 }
}

function getWeekProgress(now: Date): { remaining: number, percentage: number } {
	const day = now.getDay()
	const passed = day === 0 ? 6 : day - 1
	return { remaining: 6 - passed, percentage: ((passed + 1) / 7) * 100 }
}

function getMonthProgress(now: Date): { remaining: number, percentage: number } {
