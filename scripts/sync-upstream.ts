...
function getUpstreamBranch(): string {
  try {
    ...
    for (const b of ['upstream/main', 'upstream/master']) {
      try { run('git rev-parse --verify ' + b, true); return b } catch { /* continue */ }
    }
...
if (!branch) fatal('[ERROR] 当前处于 detached HEAD，无法同步')

try { run('git rev-parse --verify -q MERGE_HEAD', true); fatal('[ERROR] 有未完成的 merge，请先完成或 abort') } catch { /* ok */ }
try { run('git rev-parse --verify -q REBASE_HEAD', true); fatal('[ERROR] 有未完成的 rebase，请先完成或 abort') } catch { /* ok */ }
...
const oldCount = run('git stash list --format="%gd %gs"', true)
  .split('\n')
  .filter((l) => l && l.includes('sync-upstream: WIP')).length
for (let i = 0; i < oldCount; i++) {
  run('git stash drop stash@{0}', true)
}
if (oldCount > 0) log('[CLEAN]', '清理了 ' + oldCount + ' 个旧 stash')
...
if (behind === '0') {
  ...
  try { run('git merge --ff ' + upstreamBranch) }
  catch {
    console.error()
...
if (hasChanges) {
  ...
  if (!ref) fatal('[ERROR] 找不到 stash，请手动 git stash apply')
